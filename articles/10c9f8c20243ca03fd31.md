...
BigQuery を使っているほうのなかには、データパイプラインをスケジュールクエリで頑張っているケースも少なくないかと思います。スケジュールクエリは依存関係を考慮できません（実行タイミングをズラして頑張るしかない）。ワークフローエンジンを導入していない BigQuery ユーザーには、導入して損のないツールです。SQLX は SQL がわかる人であれば学習コストもほとんどかかりません。
