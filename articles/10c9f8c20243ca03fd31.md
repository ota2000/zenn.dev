...
https://docs.cloud.google.com/dataform/docs/overview?hl=ja

Dataform は、従来の ETL パイプラインではなく**ELT パイプライン**で役立つツールです。
ELT パイプラインとは、データソースから抽出（**Extract**）した生データをそのままデータウェアハウス（DWH）へ読み込み（**Load**）、DWH 内で目的に応じたテーブル構造へ変換（**Transform**）する一連の流れです。Dataform は、この Transform に特化したツールです。

## Google Cloud 傘下に

2020 年 12 月 8 日（米国時間）、Google Cloud が Dataform を買収したという[アナウンス](https://cloud.google.com/blog/ja/products/data-analytics/welcoming-dataform-to-bigquery?hl=ja)がありました。同時に、すべてのユーザーが無料で Dataform を使用できるようになりました。
...
- ソースデータとして[COVID-19 Public Datasets](https://console.cloud.google.com/marketplace/product/bigquery-public-datasets/covid19-public-data-program)（一般公開データ）を使用します
  - 大抵のユースケースでは事前にソースデータを BigQuery へ格納するパイプラインを組む必要があります（今回は省略）
...
![サービスアカウントの作成](https://storage.googleapis.com/zenn-user-upload/3qy03zb75yd2qapsf4gkbopyhlnt)
![BigQuery管理者ロールの付与](https://storage.googleapis.com/zenn-user-upload/k2oisxzi4p69jjfwrgujpd702yra)
サービスアカウントを作成したら JSON の秘密鍵をダウンロードして保管します。
※Dataform から発行される BigQuery のジョブはすべてこのサービスアカウントで実行されます。

### [Dataform] ユーザーアカウントを作成する

以下のリンクから Dataform のユーザーアカウントを作成します（※現在は Google Cloud Console に統合されています）
https://console.cloud.google.com/bigquery/dataform

### [Dataform] プロジェクトを作成する

サインアップが完了したら[新規プロジェクトを作成](https://console.cloud.google.com/bigquery/dataform)します。

1. 任意のプロジェクト名を入力する
   ![プロジェクト名の入力](https://storage.googleapis.com/zenn-user-upload/dbpi31o4hb52q3xc4btdumnhdmip)
2. 接続対象の GCP プロジェクト ID を入力する
   ![GCPプロジェクトIDの入力](https://storage.googleapis.com/zenn-user-upload/6396af2ob12nlvwqheqhz0qjvd6j)
3. データセットを作成するロケーションとサービスアカウントキーを指定して BigQuery と接続する
   ![BigQueryとの接続設定](https://storage.googleapis.com/zenn-user-upload/bulh1d0gp3ha6efx5nozlzl67dxi)

### (オプション) 任意の Git プロバイダでバージョン管理する

Dataform 上で作成するソースコードは Git でバージョン管理されます。Git 管理のプロバイダを任意のもの（GitHub、GitLab、Azure DevOps）へ移行できます。
...
https://docs.cloud.google.com/dataform/docs/overview?hl=ja
https://docs.cloud.google.com/dataform/docs/quickstart-create-workflow?hl=ja
...
![SQLXのコンパイル結果と依存関係の表示](https://storage.googleapis.com/zenn-user-upload/1hbdfw4phbl4utzy9gwdp0xb6smo)

SQLX を作成すると、右ペインでビューの作成先や依存関係、コンパイルされたクエリなどが表示されます。依存関係では`bigquery-public-data.covid19_open_data.covid19_open_data`への依存を確認でき、コンパイル結果では`${ref("covid19_open_data")}`の内容が展開されています。**PREVIEW RESULTS**でクエリ結果を確認でき（実際にBigQueryへクエリジョブが発行されるので課金量に要注意）、**CREATE VIEW**でビューを作成できます。

## パーティションテーブルを定義する SQLX を作成する

`bigquery-public-data.covid19_public_forecasts.japan_prefecture_28d`から都道府県別の感染状況予測（日別）をパーティションテーブルで作成するための SQLX を作成します。
...
2回目以降の実行時は以下のようなクエリジョブが発行され、INSERT を実行します。
...
![Dependency Treeによる依存関係の表示](https://storage.googleapis.com/zenn-user-upload/vqdzdb8mla991swd49j98bw9boz6)
...
![environments.jsonの編集画面](https://storage.googleapis.com/zenn-user-upload/wsf3f6dck07dxzwa9xjg88ems695)

**CREATE NEW SCHEDULE**から対象のタグや実行時に依存関係を含めて実行するかなどを指定します。
![スケジュールの作成画面](https://storage.googleapis.com/zenn-user-upload/f20899x1nu2ihx76mpnbmwzmhpf8)
...
- （事前にメールや Slack の通知設定を作成している場合）どのチャネルに通知を飛ばすか選択する
  - 「成功のみ」「失敗のみ」「すべて」の 3 パターンから選択できる

# おわりに

BigQuery を使っているかたのなかには、データパイプラインをスケジュールクエリで頑張っているケースも少なくないのではないでしょうか。スケジュールクエリは依存関係を考慮できません（実行タイミングをズラして頑張るしかない）。ワークフローエンジンを導入していない BigQuery ユーザーには、導入して損のないツールです。SQLX は SQL がわかる人であれば学習コストもほとんどかかりません。
また、Dataform のパイプラインは API で呼び出せるので、Cloud Composer などのワークフローエンジンを導入している場合でも一連の流れに組み込めます。

本稿では触れられませんでしたが、アサーションによるデータ品質チェックやクエリの単体テストもあります。冬休みに触ってみる価値はあると思います。
